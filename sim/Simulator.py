...
class Simulator:
    ...
    @staticmethod
    def run_experiment_set(experiment_set: ExperimentSet, mechanisms: List[Mechanism]) -> List[Tuple[Mechanism, ExperimentSet]]:
        simulated_experiment_sets: List[Tuple[Mechanism, ExperimentSet]] = []
        for mechanism in mechanisms:
            ...
            if curr_experiment_set.calculation_type == CalculationType.OUTCOME or curr_experiment_set.calculation_type == CalculationType.PATHWAY:
                ...
                for reaction_ndx in range(mechanism.solution.n_reactions):
                    experiments: List[Experiment] = curr_experiment_set.generate_conditions()
                    curr_experiment_set.add_simulated_experiments(experiments)
                ...
                raise NotImplementedError(f"{curr_experiment_set.calculation_type} does not have a simulator associated with "
                                          f"it yet.")
            simulated_experiment_sets.append((mechanism, curr_experiment_set))
