...
def load_job(job_file):
    ...
    excel_file = pd.ExcelFile(job_file, engine='openpyxl')

    for sheet_type in sheet_types:
        assert sheet_type in excel_file.sheet_names, (
            f"In job file '{job_file}', sheet name '{sheet_type} missing. This "
            f"is required for job_type '{job_type}'")
        sheet_dct = read_sheet(job_file, excel_file, sheet_type)
...
def read_sheet(excel_file_name:str, excel_file: pd.ExcelFile, sheet_type: str):

    # Define required data and read Excel file
    reqd_data = ALLOWED_SHEET_TYPES[sheet_type]
    df = pd.read_excel(excel_file, sheet_name=sheet_type, engine='openpyxl')
    # Check that required data are present
    for reqd_datum in reqd_data:
        assert reqd_datum in df, (
            f"In job file '{excel_file_name}', reqd datum '{reqd_datum}' missing from "
            f"sheet {sheet_type}.")
